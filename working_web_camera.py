#!/usr/bin/env python3
"""
Working web-based camera display with DirectShow backend
"""
...
def get_camera():
    global cap
    if cap is None:
        # Use DirectShow backend which we know works
        cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
        if cap.isOpened():
            # Test if we can read a frame
            ret, test_frame = cap.read()
            if ret and test_frame is not None:
                print(f"✅ Camera opened successfully with DirectShow backend")
                return cap
            else:
                cap.release()
                cap = None
...
def camera_thread():
    ...
    while True:
        ...
        if ret:
            ...
            cv2.putText(frame, "Press 's' for squat, 'p' for plank", (10, 90), 
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 1)
...
if __name__ == '__main__':
    print("🌐 Starting Working Web-based Camera Display")
    print("=" * 50)
